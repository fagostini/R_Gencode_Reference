...
if( !dir.exists(file.path(working.folder, species, genome)) )
    dir.create(file.path(working.folder, species, genome), recursive = TRUE)

if( species %in% "Homo_sapiens" ){
    if( !file.exists(file.path(working.folder, species, genome, paste0("gencode.v", version, ".annotation.gff3.gz"))) )
        download.file(paste0("ftp://ftp.sanger.ac.uk/pub/gencode/Gencode_human/release_", version, "/gencode.v", version, ".annotation.gff3.gz"),
                      file.path(working.folder, species, genome, paste0("gencode.v", version, ".annotation.gff3.gz")), quiet = TRUE)
    
    if( !file.exists(file.path(working.folder, species, genome, paste0("gencode.v", version, ".long_noncoding_RNAs.gff3.gz"))) )
        download.file(paste0("ftp://ftp.sanger.ac.uk/pub/gencode/Gencode_human/release_", version, "/gencode.v", version, ".long_noncoding_RNAs.gff3.gz"),
                      file.path(working.folder, species, genome, paste0("gencode.v", version, ".long_noncoding_RNAs.gff3.gz")), quiet = TRUE)
    
    if( !file.exists(file.path(working.folder, species, genome, paste0("gencode.v", version, ".tRNAs.gff3.gz"))) )
        download.file(paste0("ftp://ftp.sanger.ac.uk/pub/gencode/Gencode_human/release_", version, "/gencode.v", version, ".tRNAs.gff3.gz"),
                      file.path(working.folder, species, genome, paste0("gencode.v", version, ".tRNAs.gff3.gz")), quiet = TRUE)
    
    if( !file.exists(file.path(working.folder, species, genome, paste0(genome, ".chrom.sizes"))) )
        download.file(paste0("http://hgdownload.soe.ucsc.edu/goldenPath/", genome, "/bigZips/", genome, ".chrom.sizes"),
                      file.path(working.folder, species, genome, paste0(genome, ".chrom.sizes")), quiet = TRUE)
}else{
    if( !file.exists(file.path(working.folder, species, genome, paste0("gencode.v", version, ".annotation.gff3.gz"))) )
...
chrominfo = sortSeqlevels(with(fread(file.path(working.folder, species, genome, paste0(genome, ".chrom.sizes")))[V1%in%chromosomes,],
                               Seqinfo(V1, seqlengths = V2, genome = genome)))

TxDb = makeTxDbFromGFF(file = file.path(working.folder, species, genome, paste0("gencode.v", version, ".annotation.gff3.gz")),
                       format = "gff3",
                       dataSource = paste("Gencode version", version),
                       organism = gsub("_", " ", species),
                       chrominfo = chrominfo)

gff = import.gff3(file.path(working.folder, species, genome, paste0("gencode.v", version, ".annotation.gff3.gz")))
...
lnc.gff = import.gff3(file.path(working.folder, species, genome, paste0("gencode.v", version, ".long_noncoding_RNAs.gff3.gz")))
...
if( !dir.exists(file.path(working.folder, species, genome, "RData")) )
    dir.create(file.path(working.folder, species, genome, "RData"), recursive = TRUE)

save(genes, geneBT, gene.metadata, file=file.path(working.folder, species, genome, "RData", paste0(genome, "_Gencode", version, "_genes_annotations_minimal.RData")))
save(txs, txsBT, txs.metadata, file=file.path(working.folder, species, genome,  "RData", paste0(genome, "_Gencode", version, "_txs_annotations_minimal.RData")))
```
...
save(genes, geneBT, gene.metadata, genes.pc.granges, genes.nc.granges, exonsByGene, exons.pc.granges, exons.nc.granges,
     file=file.path(working.folder, species, genome, "RData", paste0(genome, "_Gencode", version, "_genes_annotations.RData")))
save(txs, txsBT, txs.metadata, exonsByTxs, txs.longest.pc.pre.granges, txs.longest.nc.pre.granges, txs.longest.pc.rna.granges, txs.longest.nc.rna.granges,
     file=file.path(working.folder, species, genome,  "RData", paste0(genome, "_Gencode", version, "_txs_annotations.RData")))
```

### Output

- __&lt;genome&gt;\_Gencode&lt;version&gt;\_genes_annotations_minimal.RData__: It contains the basic annotation objects, including genes GRanges, biotypes and metadata information.
- __&lt;genome&gt;\_Gencode&lt;version&gt;\_txs_annotations.RData__: It contains the basic annotation objects, including transcripts GRanges, biotypes and metadata information.
...
```{r}
if( !dir.exists(file.path(working.folder, "img")) )
    dir.create(file.path(working.folder, "img"))
...
saveRDS(txs.longest.pc.regions, file=paste0(genome, "_Gencode", version, "_annotations.pc.transcript.regions.rds"))
```
...
mcols(tx.regions)$type = factor(mcols(tx.regions)$type, levels=c("CDS", "three_prime_UTR", "five_prime_UTR", "exon", "intron"))

saveRDS(tx.regions, file=paste0(genome, "_Gencode", version, "_annotations.all.genes.regions.rds"))
```

```{r chromosomalGeneDensity}
...
gg1 = ggplot(density.dt, aes(x=chrOrder, y=Coverage, fill=bioGroup)) +
    ...
    scale_x_continuous("Chromosomes (ordered by size)", breaks=tab$chrOrder, labels=tab$seqnames) +
...
png("img/chr_density.png", units="in", height=8, width=12, res=300)
...
kable(data.frame(tab), col.names=colnames(tab), format='markdown', digits=1,
      caption="Table 4: Summary of all non-overlapping genomic regions (by gene)")
```
...
saveRDS(tx.regions, file=paste0(genome, "_Gencode", version, "_annotations.all.genes.transcript.regions.rds"))
```
